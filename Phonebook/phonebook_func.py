...
def center_window(self, w, h): #pass in the tkinter frame reference and the w and h
    # get user's screen width and height
    screen_width = self.master.winfo_screenwidth()
    screen_height = self.master.winfo_screenheight()

    # calculate x and y coordinates to pain the app centered on screen
...
# catch if user clicks upper 'x' to ensure they want to close
def ask_quit(self):
...
def first_run(self):
    data = ('John','Doe','John Doe', '555-555-5555','john@johndoe.com') #creates generic first row
    conn = sqlite3.connect('db_phonebook.db')
    with conn:
        cur = conn.cursor()
...
def onSelect(self,event):
    ...
    with conn:
        cur = conn.cursor()
        cur.execute("""SELECT col_fname, col_lname, col_phone, col_email
FROM tbl_phonebook WHERE col_fullname = (?)""", [value])
        varBody = cur.fetchall()
        # This returns a tuple and we can slice it into 4 parts
        for data in varBody:
...
def addToList(self):
    ...
    var_fname = var_fname.strip() # these take out unwanted spaces
    var_lname = var_lname.strip()
    var_fname = var_fname.title() # title ensures capitalization
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email)> 0):
        ...
        with conn:
            cursor = conn.cursor()
            # check database for existance of the fullname. if so, alert user and disregard request
            cursor.execute("""SELECT COUNT(col_fullname) FROM tbl_phonebook WHERE
col_fullname = '{}'""".format(var_fullname)),(var_fullname) #possible error on this line
            count = cursor.fetchone()[0]
            chkName = count
            if chkName == 0: # if this is 0 then the name doesn't exist yet
                print("chkName: {}".format(chkName))
                cursor.execute("""INSERT INTO tbl_phonebook
(col_fname, col_lname, col_fullname, col_phone, col_email) VALUES (?,?,?,?,?)""",
        ...
        messagebox.showerror("Missing Text Error","Please fill out all fields.")
...
def onDelete(self):
    ...
    with conn:
        ...
        # check count to ensure that this isn't the last record in db
        # cannot delete last record
        cur.execute("""SELECT COUNT(*) FROM tbl_phonebook""")
        count = cur.fetchone()[0]
        if count > 1:
            ...
            if confirm:
                ...
                onDeleted(self) # call function to clear all the textboxes and selected index of listbox
                onRefresh(self) # update the listbox
